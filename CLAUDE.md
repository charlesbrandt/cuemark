...
- **GStreamer** (Rust, `gstreamer` + `gstreamer-audio` crates, `features = ["v1_18"]` required) — audio playback, gain/EQ, device routing, headphone cue mix, recording. Each deck has its own `DeckAudioPipeline`; full topology in "Audio pipeline" under Architecture below. Audio is the master clock; video element syncs to it.
...
script/probe to reach for and when, one row per script — moved out of this file
2026-09-05 to cut lookup-table bulk from the main context.
