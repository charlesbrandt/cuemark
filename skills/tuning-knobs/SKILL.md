...
| | |
|---|---|
| **Where** | `autoMix.ts`: `MIN_ZONE_SEC` (2), `MIN_TRANSITION_MS` / `MAX_TRANSITION_MS` (2 s / 20 s), `INTRO_SEEK_MIN_SEC` (1 s), `PREVIEW_TAIL_MS` (3 s), plus the Settings blend length / threshold / preload. |
...
| **🛑 Known trap** | With a usable outro zone the blend **starts at `mixOutStart`** (since 2026-09-19) and the "start mixing N s out" threshold no longer moves it; it still governs tracks with no usable outro zone. Turning that setting and seeing nothing change on a marked track is by design. |
| **Point rule vs. zone rule** | Two separate gates per side since 2026-09-20, both resolved by the single exported `effectiveZones()`. **POINT** (`inStart`/`outStart`) — is this a plausible place to start/fade from: mix-in within the first third of the track, mix-out not within it. **ZONE** (`inLenSec`/`outLenSec`) — is this a usable *blend length*: needs both ends placed and at least `MIN_ZONE_SEC` (2 s) between them. A value can pass one and fail the other — e.g. a 0.5 s mix-in is a fine start point and a useless zone — which used to be one gate that rejected both together. |
| **Marker panel says "ignored"** | `MarkerPanel` shows `effectiveZones()`'s own `inRejected`/`outRejected` string verbatim instead of a bare "ignored": `mix-in past the first third` / `mix-out inside the first third` (POINT rule failed), `no end marker` (a start was trusted but its `_end` isn't placed), or `zone under 2s` (`MIN_ZONE_SEC`). |
...
| **🛑 The intro side is inert on an un-marked library** | Since 2026-09-19 the incoming deck is *seeked* to `effectiveZones().inStart` before it starts. That seek is gated on two separate things, not one: the POINT rule above (which Digger's auto `mix_in_start` — the first tracked beat, well under a second — passes fine) and then `INTRO_SEEK_MIN_SEC` (1 s). The second gate is a **delivery** floor, not a trust rule — it declines to spend a GStreamer pipeline + codec-decoder flush + seek-settle on a fraction of a second of near-silence, not because the point is untrusted. The observable behaviour is unchanged: still inert on an un-marked library, still **hand-place a mix-in in the MarkerPanel before concluding this is broken** — only the mechanism moved. The log tells you which happened: `[auto-dj] intro: deck-N starting at its mix-in marker …` fires only when both gates pass, and the `[auto-dj] preview:` line always names the incoming deck's start as either `…s (mix-in)` or `0.0s (no mix-in)`. |
