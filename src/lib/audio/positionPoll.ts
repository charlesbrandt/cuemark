...
import { checkAutoMixTrigger, checkAutoPreloadTrigger } from "../digger/autoMix";
...
export function pollDeckPosition(
  ...
  audioGetPosition(deck.id).then((sample) => {
    ...
    // Preload fires first (earlier threshold) so the incoming deck has a chance to report a
    // real duration before the crossfade trigger goes looking for one.
    if (!scratching) {
      checkAutoPreloadTrigger(capturedDeckId, contentPos);
      checkAutoMixTrigger(capturedDeckId, contentPos);
    }
